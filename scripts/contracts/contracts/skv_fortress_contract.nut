this.skv_fortress_contract <- this.inherit("scripts/contracts/contract", {
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/skv_grandlodge.png[/img]{The lodge is two rooms over a chandler's and smells of it. %SKVNAME%Venture-Captain Ambrus Valsin%SKVNAME_OFF% does not stand up when you come in, and does not stop writing for a while after that.%SPEECH_ON%A month ago the ground moved and one of the old siege castles out on the plain came open. Sealed since before anybody's grandfather, and now there is a hole in the side of it you could drive a cart through.%SPEECH_OFF%He puts the pen down.%SPEECH_ON%I have a man in there. Balenar Forsend. He went out to make a name and he did not tell me he was going, which is the sort of thing that gets written on a stone. That was a fortnight past.%SPEECH_OFF%%SPEECH_ON%The tower is not empty. Something has moved into it and it was not scholars. I want him out, and I would rather not send another of my own in after him to find out what the first one found.%SPEECH_OFF%}",
			Image = "",
			List = [],
