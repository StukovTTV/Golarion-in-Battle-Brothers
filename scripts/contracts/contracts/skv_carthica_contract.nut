...
//  Built on ::Skv: Once (offer lock), Check (brawn/agility/handEye/nerve/guile + scaledBase),
//  XP (won contests + goal-scaled at completion), Loot. ART: event_NN placeholders (doc §8).
// ============================================================================
this.skv_carthica_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Deck        = null,   // the fixed room sequence, assembled when the crawl begins
		CrawlIndex  = 0,      // which card you are on
		Points      = 0,      // popularity meter -- CAN GO NEGATIVE (contest losses; serialize +50)
		Wins        = 0,      // contests won (the 3-of-5 grudging-win fallback)
		GotRing     = false,  // the signet recovered -> employer goal #1
		Humiliated  = false,  // thieves publicly humbled -> won the room (Points>=7) -> goal #2
		Discreet    = false,  // house's name kept clean -- went in cloaked -> goal #3
		HasLetter   = false,  // Pictor's letter looted off Urie (warn-noble bonus / future hook)
		HasCloak    = false,  // a Sczarni cloak looted off Urie -> enables the cloaked entry
		Betrayed    = false,  // sold Carthica to the Sczarni (the betrayal fork)
		Done        = false,  // crawl concluded -> report shown
		Decided     = false,  // an ending was chosen (warn-noble recap safety)
		OutcomeText = "",     // Outcome / betrayal narrative (transient, not serialized)
		OutcomeRows = null,   // Outcome iconed rows (transient)
		RoomRows    = null,   // a room/contest result's rows (transient)
		RoomText    = "",     // a room/contest result's narrative (transient)
		ActorName   = "",     // last brother to act on a check -> ::Skv.Check %actor% (transient)
		FightWon    = false,  // the alley fight was won -> update() advances (transient)
		FightFled   = false,  // retreated from the alley fight -> re-show it (transient)
		SpottedTail = false,  // made the Sczarni tail before the alley -> forewarned, -10% fight budget
		LastContest = "",     // which contest just resolved -> themed flourish ("arm"/"rail"/...) (transient-ish)
	},
	...
	function create()
	{
		...
		this.m.Category = this.Const.Contracts.Categories.Hunt;   // you HUNT the thieves who took the ring
	...
	function start()
	{
		// Random difficulty (decision #7): 0.75-1.20 -> a variable skull rating run to run; the
		// same mult scales the checks (via ::Skv.Check.scaledBase) and the one alley fight's budget.
		this.m.DifficultyMult = this.Math.rand(75, 120) * 0.01;

		// Notable noble job. The reward is the fee + the goal-scaled XP; the ring is the point.
		...
		// Advance is SMALL and NOT guaranteed (the watchtower pattern): a proud lord paying for
		// discretion mostly holds the coin until the ring is in his hand. 1-in-3 he fronts a fifth.
		...
		{
			this.m.Payment.Completion = 1.0;   // nothing up front
		}

		this.contract.start();
	}

	// Lady Lilianna's price to name the Urgent Messenger -- meaningful, and scaled by the job's
	// difficulty (a hotter job, a dearer whisper). Deterministic per contract (no random roll), so
	// the label and the charge always match. The alternative is a secret (a brother's mood).
	function brokerFee()
	{
		return 250 + this.Math.round(300 * this.getDifficultyMult());   // ~475-610
	}
	...
	function createStates()
	{
		this.m.States.push({
			...
			function end()
			{
				// No spawn, no marker: the crawl runs in-town. Just go active; Running.update()
				// opens the first beat the moment we are standing in the hiring town (i.e. now).
		...
		this.m.States.push({
			...
			function update()
			{
				// --- The alley fight was WON: consume it, recover the letter + cloak, show the aftermath.
				if (this.Contract.m.FightWon)
				{
					this.Contract.m.FightWon = false;
					this.Contract.recoverAlley();
					this.Contract.m.CrawlIndex = this.Contract.m.CrawlIndex + 1;   // consume the "alley" card
					this.Contract.setScreen("AlleyAfter");
					this.World.Contracts.showActiveContract();
					return;
				}

				// --- Retreated from the (mandatory) alley fight: re-show the card; index NOT advanced.
				...
				if (this.Contract.m.Done) return;   // concluded (or waiting on nothing)

				// --- Begin the crawl the moment we are in the hiring town (which is where the contract
				// was just accepted). No travel, no marker. Deck==null is the "not started" latch.
	...
	// The current card, or "Finale" once the deck is spent.
	function nextScreen()
	{
	...
	// Stash a non-contest room's outcome (broker reply, search result, door choice), CONSUME the
	// card, and route to the shared RoomResult screen so the outcome is actually SHOWN before the
	// next beat -- instead of silently jumping on (the "nothing tells the story" bug).
	...
	// A short between-beats street vignette (doc §10) -- pure table colour.
	function vignette()
	{
		local pool = [
	...
	// Themed crowd-work per contest (#4, from the source: each bonus needles the thief who just lost/won
	// that game). Returns { opt, win, lose } for the flourish screen, keyed on m.LastContest.
	...
	// Resolve a contest feat: win = +2 & a Win (+XP to the actor); loss = -1. Records which contest
	// (_key) for the themed flourish, stashes the result, consumes the card, routes to the flourish.
	function resolveContest( _res, _winText, _loseText, _key )
	{
		...
		this.m.CrawlIndex = this.m.CrawlIndex + 1;   // the contest card is spent; the flourish belongs to it
	...
	function newFightProperties()
	{
		...
		p.LocationTemplate.Template[0] = "tactical.ruins";   // a garbage-choked back-alley read, not open field
	...
	// Urie's ambush: a Sczarni knot -- scaled off the skull rating AND company strength, so it
	// stays a real fight for a strong band without flooring a weak one.
	function startAlleyFight()
	{
		...
		// Base 73, times DIFFICULTY scaling (getDifficultyMult) AND COMPANY-strength scaling
		// (getScaledDifficultyMult) -- buffed from 45 for a meatier alley fight; keep both multipliers.
		local budget = 73 * this.getDifficultyMult() * this.getScaledDifficultyMult();
		if (this.m.SpottedTail) budget = budget * 0.9;   // forewarned by spotting the tail -> a lighter ambush
	...
	// After Urie falls: his body carries a Sczarni cloak (enables the cloaked tavern entry) and
	// Pictor's letter (the warn-noble bonus + a future hook). Builds the AlleyAfter rows/text.
	...
	// The noble house's faction, for the completion relation. Guarded.
	...
	function createScreens()
	{
		...
		// --- TASK: Natasha the fixer + Carthica, in a coarse tavern (the kept fiction, #15).
		this.m.Screens.push({
			ID = "Task",
		...
		// --- LORE: two brothers on the Sczarni -- accurate to the Varisian crime families.
		this.m.Screens.push({
			ID = "Lore",
			Title = "The Sczarni",
		...
		// --- BROKER (Act II): Lady Lilianna. Two prices -- coin (scaled), or a secret (a mood hit). (#3)
		this.m.Screens.push({
			ID = "BrokerRoom",
			Title = "Lady Lilianna",
			Text = "",
		...
		// --- DOCKS (wrong-boat gag, #7 / source "The Wrong Boat"): the party humours the notion that
		//     the Urgent Messenger is a SHIP. It is not. Pure comedy; loops back to the broker. No cost.
		this.m.Screens.push({
			ID = "Docks",
		...
		// --- TAIL (Act III pre-beat, #3): spot the Sczarni shadowing you before the ambush. A
		//     Perception success -> forewarned (SpottedTail), which trims the alley fight budget 10%.
		this.m.Screens.push({
			ID = "TailRoom",
			Title = "Watched",
			Text = "",
		...
		// --- GARBAGE (Act III hazard, #7): the choked passage to the courtyard. A PER-BROTHER agility
		//     roll -- the nimble keep their feet and their gorge; those who fail catch the real Legends
		//     "sick" injury (getSkills().add(sickness_injury), exactly as the swamp travel event does),
		//     which then CARRIES INTO the alley fight as a debuff. Sickened brothers narrated per name.
		this.m.Screens.push({
			ID = "GarbageRoom",
		...
		// --- ALLEY (Act III): the one fight. Urie -- a retained Sczarni executioner -- offers the
		//     betrayal instead. Transparent, coded, no malice (accurate to the lore). (#4)
		this.m.Screens.push({
			ID = "AlleyRoom",
		...
		// --- ALLEY AFTERMATH: recover the cloak + Pictor's letter, then on to the courtyard.
		this.m.Screens.push({
			ID = "AlleyAfter",
			...
			Options = [
				{
					// The alley card was ALREADY consumed by update() (CrawlIndex++ on victory); just
					// show the CURRENT card -- calling advance here would skip the courtyard.
		...
		// --- BETRAYAL ENDING: sell the noble; keep advance + bribe, lose standing + morale. Concludes here.
		this.m.Screens.push({
			ID = "Betray",
			Title = "A Kinder Deal",
			Text = "",
			...
			Options = [
				{
					...
					function getResult()
					{
						this.World.Contracts.finishActiveContract(true);   // the noble's job failed, by our hand
						return 0;
					}
				}
			],
		...
		// --- COURTYARD (Act III pt.2): find the hidden door -- perception, or bribe the bums (#5).
		this.m.Screens.push({
			ID = "CourtyardRoom",
				...
				this.Options = [
					{
						...
						function getResult()
						{
							...
							// Failed the read -> fall through to the bribe (no hard block).
		...
		// --- COURTYARD BRIBE: the fallback after a failed search -- pay, and in you go.
		this.m.Screens.push({
			ID = "CourtyardBribe",
			Title = "The Dead Courtyard",
			Text = "",
		...
		// --- DOOR: at the threshold, the hide-Carthica's-identity choice (#12) -> sets Discreet.
		this.m.Screens.push({
			ID = "DoorRoom",
			Title = "The Threshold",
			Text = "",
			...
			function start()
			{
				...
				this.Options.push({
					...
					function getResult()
					{
						...
						this.Contract.m.Points = this.Contract.m.Points - 2;   // a colder room to start (doc §5/§6)
		...
		// --- THE FIVE CONTESTS (Act IV). Each: a feat check (its flavor) -> the shared flourish screen.
		this.m.Screens.push({
			ID = "ArmRoom",
				...
				this.Options = [
					{
						...
						function getResult()
						{
							local res = ::Skv.Check.brawn(this.Contract, 50);   // flat 50, no difficulty scaling
							local who = (res.actor != null) ? res.actor.getName() : this.Contract.m.ActorName;
				...
				this.Options = [
					{
						...
						function getResult()
						{
							local res = ::Skv.Check.agility(this.Contract, 50);   // flat 50, no difficulty scaling
							local who = (res.actor != null) ? res.actor.getName() : this.Contract.m.ActorName;
							return this.Contract.resolveContest(res,
				...
				this.Options = [
					{
						...
						function getResult()
						{
							local res = ::Skv.Check.handEye(this.Contract, ::Skv.Check.scaledBase(this.Contract, 50));   // base 50, difficulty-scaled
							local who = (res.actor != null) ? res.actor.getName() : this.Contract.m.ActorName;
							return this.Contract.resolveContest(res,
				...
				this.Options = [
					{
						...
						function getResult()
						{
							local res = ::Skv.Check.nerve(this.Contract, 50);   // flat 50, no difficulty scaling
							local who = (res.actor != null) ? res.actor.getName() : this.Contract.m.ActorName;
							return this.Contract.resolveContest(res,
				...
				this.Options = [
					{
						...
						function getResult()
						{
							local res = ::Skv.Check.guile(this.Contract, ::Skv.Check.scaledBase(this.Contract, 50));   // base 50, difficulty-scaled
							local who = (res.actor != null) ? res.actor.getName() : this.Contract.m.ActorName;
							return this.Contract.resolveContest(res,
		...
		// --- SHARED FLOURISH: after each feat -- an optional, RISKY guile play to the crowd (#13).
		this.m.Screens.push({
			ID = "ContestFlourish",
			...
			function start()
			{
				...
				this.List = this.Contract.m.RoomRows != null ? this.Contract.m.RoomRows : [];
				local f = this.Contract.flourishFor();   // themed to the contest that just resolved (#4)
				this.Options = [
					{
						Text = f.opt,
						function getResult()
						{
							local res = ::Skv.Check.charm(this.Contract, ::Skv.Check.scaledBase(this.Contract, 50));   // crowd-work = charm; base 50 scaled
							local fr = this.Contract.flourishFor();   // same key -> same lines
		...
		// --- FLOURISH RESULT: shows the crowd-work outcome, then continues to the next contest / finale.
		this.m.Screens.push({
			ID = "FlourishResult",
			Title = "Working the Room",
			Text = "",
		...
		// --- ROOM RESULT: shared "show a non-contest outcome, then continue" screen.
		this.m.Screens.push({
			ID = "RoomResult",
		...
		// --- FINALE: branch on the meter, set the goal flags, then report to Carthica (no travel).
		this.m.Screens.push({
			ID = "Finale",
			Title = "The Reckoning",
			Text = "",
		...
		// --- REPORT: Carthica, in the same town -- paid & judged on the three goals; goal-scaled XP (#12).
		this.m.Screens.push({
			ID = "Report",
			...
			function start()
			{
				...
				{
					// Soft fail: no completion pay, a BIG renown loss, a relation hit. Concludable, but it stings.
		...
		// --- OUTCOME: the warn-noble recap. Finishes on dismiss.
		this.m.Screens.push({
			ID = "Outcome",
			Title = "Carthica's Due",
			Text = "",
	...
	function onClear()
	{
		::Skv.Once.release("Carthica");   // always free the live-offer slot (any removal)
		if (this.m.IsActive)
		{
			::Skv.Once.retire("Carthica");   // accepted then concluded -> retire for the campaign
		}
	}
	...
	function onSerialize( _out )
	{
		...
		// Points can go NEGATIVE (contest losses, the open-door penalty). writeU8 is unsigned, so
		// bias by +50 out and undo it in (range ~ -50..+50 -> 0..100).
