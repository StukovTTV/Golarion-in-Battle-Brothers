// ============================================================================
//  BLACK FORKS -- CONTRACT (state machine)
//
//  Investigate the fires at the ruined monastery of Black Forks. The druids who
//  hold rites on its roof meet you at the gate and warn you off: cultists (the
//  Tenders) have taken the halls. You press in.
//
//  Day  -> a smallish band of cultists (Spawn.Cultists), no leader.
//  Night-> the full cult at rite: a named champion leader + an auto-scaling
//          militia->mercenary retinue + a body of cultists (Spawn.GolarionCult).
//
//  You report back to the village for pay. Clearing them by DAY pays the base.
//  Clearing them at NIGHT -- catching the cult mid-rite and learning they are
//  spreading -- pays DOUBLE, but leaves the village Terrified (they now know
//  what is loose in the wood).
//
//  Structure: Offer -> Running (arrive -> Approach -> fight -> report back).
//  The "report leg" is done with a flag inside Running (return_item style),
//  not a separate state.
// ============================================================================
this.skv_black_forks_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Destination = null,
		WasNight    = false,   // set at combat; picks the day/night report + payout
	...
	function start()
	{
		// Standard-ish rating; can tip to 3 skulls on a high roll (a champion-led
		// night fight earns it). Feeds the payout via DIFF^POW and the skulls.
		this.m.DifficultyMult = this.Math.rand(90, 120) * 0.01;

		// Base 300, times a wealth factor read from the hiring village's actual
		// resources (situations move these when WorldEconomy is on; with it off it
		// sits near 1.0, which is correct -- that player opted out of wealth play).
		// Poorer village pays less, richer pays more. Clamped 0.5-1.5.
		local wealth = 1.0;
		...
		// Paid on results at the village (no advance). The night bonus is a second
		// full pool added at the ReportNight screen.
	...
	function createStates()
	{
		this.m.States.push({
			...
			function end()
			{
				...
				// Bandits faction: cultists are bandit-faction in combat, and we run
				// our own scripted fight regardless. setAttackable(false) so the site
				// is a marker we resolve via the Approach screen, not a garrison.
		...
		this.m.States.push({
			...
			function update()
			{
				// --- REPORT LEG: site cleared, walk back to the village for pay ----
				...
				// --- Victory just happened (flag set in the combat callback) -------
				// Flip to the report leg here, on the tick AFTER combat, never inside
				// the callback (that stacks a screen on the tearing-down combat UI).
				...
				// --- Arrival at the monastery: the druids meet you (Approach) ------
				// The ROSTER branches on IsDaytime in onCombat, so the arrival screen
				// must TELL the player which fight they are walking into -- otherwise
				// the night gamble is a hidden trap rather than a decision.
				if (this.Contract.isPlayerAt(this.Contract.m.Destination))
				{
					local isDay = this.World.getTime().IsDaytime;
					// Re-arm when day flips to night (or back) while the party waits at
					// the site, so someone who "holds back for dawn" gets the new, true
					// briefing instead of a stale one.
			...
			function onCombat()
			{
				...
				// Dress the field as the ruined monastery.
				...
				// The whole fight turns on WHEN you strike. Read it here (at the fight),
				// stash it for the report/payout.
				this.Contract.m.WasNight = !this.World.getTime().IsDaytime;

				if (this.Contract.m.WasNight)
				{
					// Night: the cult at rite -- champion leader + scaling retinue + body.
					::Const.World.Common.addUnitsToCombat(p.Entities, ::Const.World.Spawn.GolarionCult, this.Contract.m.NightBudget * mult, fac);
				}
				else
				{
					// Day: a smallish band of cultists, no leader.
			...
			// Flag only -- update() shows the report leg next tick (deferral rule).
			...
			// Fleeing does NOT clear the site: it stays, you can go back and re-approach.
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			...
			ShowDifficulty = true,
			Options = [],
			// Options rebuilt WHOLESALE each time this screen shows (start() re-runs on
			// return from "Lore", so pushing would duplicate). Lore option hidden below a
			// roster of 2, because %randombrother2% renders "unknown" then.
			function start()
			{
				this.Options = [
					{
		...
		// ---------------------------------------------------------------------------
		// LORE SCREEN -- opt-in worldbuilding, delivered as a conversation between
		// %randombrother% and %randombrother2% (contract.nut:319-326 guarantees they are
		// two DIFFERENT men; repeating %randombrother% gives the SAME man). Returns to
		// "Task", so the offer is untouched. Accurate to the canon of the site.
		// ---------------------------------------------------------------------------
		this.m.Screens.push({
			ID = "Lore",
			Title = "Black Forks",
		...
		// --- Two arrival screens. The druids are the telegraph: they tell you plainly
		// which fight is waiting, so "Hold back for now" is an informed choice and the
		// double pay at night is a reward for known risk, not a lucky accident. -------
		this.m.Screens.push({
			ID = "ApproachDay",
			Title = "Black Forks",
			...
			Options = [
				{
					...
					{
						this.World.Assets.addBusinessReputation(this.Const.World.Assets.ReputationOnContractSuccess);
						// Base + a second full pool: catching them mid-rite pays double.
						this.World.Assets.addMoney(this.Contract.m.Payment.getOnCompletion());
						...
						// The village now knows what is loose in the wood.
						if (this.Contract.m.Home != null && !this.Contract.m.Home.isNull())
						{
	...
	function onClear()
	{
		::Skv.Once.release("BlackForks");   // always free the live-offer slot (any removal)
		if (this.m.IsActive)
		{
			::Skv.Once.retire("BlackForks");   // accepted then concluded (completed/aborted) -> retire
	...
	{
		// Location highlighting. Same seam and same reasoning as the Den event's
		// %SKVNAME%: Legends' own %OOC% is nothing but a var that expands to a
		// colour tag, and onPrepareVariables is the shipped place to add your own
		// (mod_legends/hooks/contracts/contract.nut, vars table ~342-440).
		// Delivering it by substitution means the tag reaching the renderer is
		// byte-identical to the one %SPEECH_ON% injects, which demonstrably works.
		//
		// WHY VIOLET. Every colour BB already spends is one hue with three
		// brightness steps -- body #bd9d71 (hue 35), speech #bcad8c (41), OOC
		// #f6eedb (42), all sampled from a real screenshot. Separating inside that
		// family means separating by luminance, which is what speech does, and
		// speech is only ~12/255 off the body -- effectively invisible. So a mark
		// has to leave the hue. Names took blue-grey #9dbccb (hue 200). Violet
		// #b39dbc (hue 283) is the other far point, and BB spends no violet
		// anywhere, so it cannot collide with an existing meaning the way a green
		// brushes against Const.UI.Color.PositiveEventValue.
		//
